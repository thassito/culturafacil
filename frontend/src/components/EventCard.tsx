    ...
    <div className="bg-gray-800/50 backdrop-blur-md rounded-lg border border-gray-700/50 overflow-hidden hover:border-blue-500 transition-colors duration-300">
      ...
      <div className="p-4">
        ...
        <div className="flex flex-wrap gap-2 mb-2">
          {event.tags.map(tag => (
            <span key={tag} className="px-2 py-1 bg-blue-600/50 text-blue-200 text-xs font-semibold rounded-full">
              {tag}
            </span>
          ))}
        </div>
        <h3 className="text-lg font-bold text-white truncate" title={event.name}>{event.name}</h3>
        <p className="text-sm text-gray-400 mt-1">{event.date} &bull; {event.location}</p>
        <div className="mt-4">
